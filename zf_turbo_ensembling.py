...
def filter_boxes_v2(boxes, scores, labels, thr):
    ...
    for t in range(len(boxes)):
        cur_box_lst = boxes[t]
        for i in range(len(cur_box_lst)):
            box = []
            cur_box_arr = cur_box_lst[i]
            for j in range(cur_box_arr.shape[0]):
                label = labels[t][i][j].astype(np.int64)
                score = scores[t][i][j]
                if score < thr:
                    break
                if i == 0:
                    b = [int(label), float(score),
                         float(cur_box_arr[j, 0]), float(cur_box_arr[j, 1]),
                         float(cur_box_arr[j, 2]), float(cur_box_arr[j, 3])]
                else:  # Mirror fix !!!
                    assert i == 1
                    raise AssertionError('Mirror')
                    b = [int(label), float(score),
                         1 - float(cur_box_arr[j, 2]), float(cur_box_arr[j, 1]),
                         1 - float(cur_box_arr[j, 0]), float(cur_box_arr[j, 3])]
                box.append(b)
            # box = np.array(box)
            new_boxes.append(box)
    return new_boxes
